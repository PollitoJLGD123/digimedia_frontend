...
export default function Footer({ id_blog_footer }) {
    ...
    if (isLoading) {
        return (
            <div className="mt-12 max-w-[1000px] mx-auto bg-gradient-to-r from-gray-900 to-gray-800 rounded-lg shadow-[0px_8px_20px_rgba(0,0,0,0.25)] overflow-hidden animate-pulse">
                    ...
                    <div className="p-6 md:p-8">
                        ...
                        <div className="flex flex-wrap justify-center gap-3 mt-6">
                            {[1, 2, 3].map((i) => (
                                <div
                                    key={i}
                                    className="w-48 h-36 bg-gradient-to-r from-gray-700 to-gray-600 rounded-lg border border-gray-600"
                                ></div>
                            ))}
                        </div>
                        ...
                        <div className="flex justify-center mt-6">
                            <div className="w-12 h-0.5 bg-gradient-to-r from-white via-sky-400 to-white rounded-full"></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    if (error) {
        return (
            <div className="mt-12 max-w-[1000px] mx-auto bg-gradient-to-r from-gray-900 to-gray-800 rounded-lg shadow-[0px_8px_20px_rgba(0,0,0,0.25)] overflow-hidden">
                <div className="relative">
                    <div className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-red-500 via-yellow-400 to-blue-500"></div>
                    ...
                    <div className="p-6 md:p-8 flex flex-col items-center">
                        ...
                        <h3 className="text-2xl md:text-3xl text-center font-bold mb-4 text-yellow-400">
                            ...
                            <span className="block h-0.5 w-16 bg-yellow-400/50 mx-auto mt-2"></span>
                        </h3>
                        ...
                        <button
                            ...
                            className="px-6 py-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-gray-900 rounded-lg font-medium hover:from-yellow-500 hover:to-yellow-600 transition-colors"
                        >
                            Reintentar
                        </button>
        ...
        <div className="mt-12 max-w-[1000px] mx-auto bg-gradient-to-r from-gray-900 to-gray-800 rounded-lg shadow-[0px_8px_20px_rgba(0,0,0,0.25)] overflow-hidden">
            <div className="relative">
                ...
                <div className="p-6 md:p-8">
                    ...
                    <p className="text-gray-100 text-base leading-relaxed max-w-3xl mx-auto mb-6">{data.descripcion}</p>

                    {(data.url_image1 || data.url_image2 || data.url_image3) && (
                        <div className="flex flex-wrap justify-center gap-3 mt-6">
                            {[data.url_image1, data.url_image2, data.url_image3].map((image, index) => {
                                const imageUrl = image
                                    ? (image.startsWith("http") ? image : `/blog/${image}`)
                                    : "/blog/blog-2.jpg"; 

                                return (
                                    <div key={index} className="relative group">
                                        <div className="absolute inset-0 bg-gradient-to-r from-red-500/20 to-blue-500/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                                        <img
                                            src={imageUrl}
                                            alt={`Imagen ${index + 1}`}
                                            className="w-48 h-36 object-cover rounded-lg border border-white group-hover:border-sky-400 transition-all duration-300 shadow-md"
                                            loading="lazy"
                                        />
                                    </div>
                                );
